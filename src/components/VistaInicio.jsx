...
const DICCIONARIO_AVATARES = {
  'batman': batman, 'spiderman': spiderman, 'ariel': ariel,
  'capitanAmerica': capitanAmerica, 'cenicienta': cenicienta,
  'rapunzel': rapunzel, 'superman': superman, 'wonderwoman': wonderwoman,
  'default': batman 
};
const CODIGOS_DISPONIBLES = Object.keys(DICCIONARIO_AVATARES).filter(k => k !== 'default');
...
function VistaInicio({ onIniciar }) {
  ...
  const manejarGuardarNuevo = async () => {
    ...
    if (nuevo) {
      await cargarDatos();
      if (creandoPara === 'p1') setP1Id(nuevo.id_usuario.toString());
      if (creandoPara === 'p2') setP2Id(nuevo.id_usuario.toString());
      cerrarModal();
    }
  };
  ...
  const manejarInicioJuego = () => {
    ...
    if (usuarioP1.id_usuario === usuarioP2.id_usuario) return alert("¡Elijan distintos!");
    
    onIniciar({
      p1: { nombre: usuarioP1.nickname, avatar: DICCIONARIO_AVATARES[usuarioP1.avatar_code], id_usuario: usuarioP1.id_usuario },
      p2: { nombre: usuarioP2.nickname, avatar: DICCIONARIO_AVATARES[usuarioP2.avatar_code], id_usuario: usuarioP2.id_usuario },
    });
  };
    ...
    <div className="contenedor-inicio">
      ...
      <header className="header-principal">
        <div className="badge-escuela">
          <div className="icono-badge"><span className="material-symbols-outlined">sports_score</span></div>
          ...
          <div className="contenido-tarjeta">
            ...
            <div className="fila-input">
              <select value={p1Id} onChange={(e) => setP1Id(e.target.value)}>
                ...
                {listaUsuarios.map(u => (
                  <option key={u.id_usuario} value={u.id_usuario}>{u.nickname}</option>
                ))}
              </select>
              <button onClick={() => abrirModal('p1')} className="btn-nuevo rosa" title="Crear Nuevo">+</button>
            </div>

            <div className="area-avatar rosa">
              {usuarioP1 ? (
                 <div className="avatar-display">
                   <img src={DICCIONARIO_AVATARES[usuarioP1.avatar_code]} alt="Avatar" />
                   <h3>{usuarioP1.nickname}</h3>
                   <span className="badge-nivel">NIVEL {Math.floor(usuarioP1.puntos_totales/1000) + 1}</span>
                 </div>
              ) : (
        ...
        <div className="tarjeta-jugador verde">
          ...
          <div className="contenido-tarjeta">
            ...
            <div className="fila-input">
              ...
              <button onClick={() => abrirModal('p2')} className="btn-nuevo verde" title="Crear Nuevo">+</button>
            </div>

            <div className="area-avatar verde">
              {usuarioP2 ? (
                 <div className="avatar-display">
                   <img src={DICCIONARIO_AVATARES[usuarioP2.avatar_code]} alt="Avatar" />
                   <h3>{usuarioP2.nickname}</h3>
                   <span className="badge-nivel">NIVEL {Math.floor(usuarioP2.puntos_totales/1000) + 1}</span>
                 </div>
              ) : (
                 <div className="avatar-placeholder">
                   <span className="material-symbols-outlined">account_circle</span>
        ...
        <div className="modal-fondo">
          <div className="modal-caja">
            ...
            <div className="botones-modal">
              ...
              <button onClick={manejarGuardarNuevo} className="btn-guardar">¡GUARDAR!</button>
            </div>
          </div>
        </div>
      )}
